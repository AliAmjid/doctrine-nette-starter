...
(function (global, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    var _global$Nette;
    let init = !((_global$Nette = global.Nette) !== null && _global$Nette !== void 0 && _global$Nette.noInit);
  ...
  Nette.getValue = function (elem) {
    if (elem instanceof HTMLInputElement) {
      if (elem.type === 'radio') {
        var _expandRadioElement$f;
        return ((_expandRadioElement$f = expandRadioElement(elem).find(input => input.checked)) === null || _expandRadioElement$f === void 0 ? void 0 : _expandRadioElement$f.value) ?? null;
      } else if (elem.type === 'file') {
        return elem.files;
      } else if (elem.type === 'checkbox') {
        return elem.name.endsWith('[]') // checkbox list
        ? expandRadioElement(elem).filter(input => input.checked).map(input => input.value) : elem.checked;
      } else {
        return elem.value.trim();
      }
    } else if (elem instanceof HTMLSelectElement) {
      var _elem$selectedOptions;
      return elem.multiple ? Array.from(elem.selectedOptions, option => option.value) : ((_elem$selectedOptions = elem.selectedOptions[0]) === null || _elem$selectedOptions === void 0 ? void 0 : _elem$selectedOptions.value) ?? null;
      ...
      if (rule.condition && success) {
        ...
        if (!onlyCheck) {
          let arr = Array.isArray(rule.arg) ? rule.arg : [rule.arg],
            message = rule.msg.replace(/%(value|\d+)/g, (foo, m) => Nette.getValue(m === 'value' ? curElem : elem.form.elements.namedItem(arr[m].control)));
  ...
  Nette.validateForm = function (sender, onlyCheck = false) {
    ...
    for (let elem of Array.from(form.elements)) {
      if (elem.getAttribute('data-nette-rules') && (!scope || elem.name.replace(/]\[|\[|]|$/g, '-').match(scope)) && !Nette.isDisabled(elem) && !Nette.validateControl(elem, null, onlyCheck) && !Nette.formErrors.length) {
        return false;
      }
    }
  ...
  Nette.isDisabled = function (elem) {
    if (elem.type === 'radio') {
      return expandRadioElement(elem).every(input => input.disabled);
    }
  ...
  Nette.validateRule = function (elem, op, arg, value) {
    ...
    args = args.map(arg => {
      if (arg !== null && arg !== void 0 && arg.control) {
        let control = getFormElement(elem.form, arg.control);
        return control === elem ? value.value : Nette.getEffectiveValue(control, true);
      }
      return arg;
    });
    return Nette.validators[method] ? Nette.validators[method](elem, Array.isArray(arg) ? args : args[0], value.value, value) : null;
  };
  Nette.validators = {
    ...
    pattern: function (elem, arg, val, newValue, caseInsensitive) {
      ...
      try {
        ...
        return val instanceof FileList ? Array.from(val).every(file => regExp.test(file.name)) : regExp.test(val);
      } catch {} // eslint-disable-line no-empty
    },
  ...
  Nette.toggleControl = function (elem, rules, success, firsttime, value = null, emptyOptional = null) {
    ...
    for (let rule of rules) {
      ...
      if (rule.condition && Nette.toggleControl(elem, rule.rules, curSuccess, firsttime, value, rule.op === ':blank' ? false : emptyOptional) || rule.toggle) {
        has = true;
        if (firsttime) {
          expandRadioElement(curElem).filter(el => !toggleListeners.has(el)).forEach(el => {
  ...
  Nette.initOnLoad = function () {
    Nette.onDocumentReady(() => {
      Array.from(document.forms).forEach(form => Nette.initForm(form));
